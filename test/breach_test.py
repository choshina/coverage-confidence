...
for ph in phi_str:
	for cp in controlpoints:
		for opt in optimization:
			property = ph.split(';')
			filename = model+ '_breach_' + property[0]+'_' + str(cp)  +'_' + timeout + '_' +  opt 
			param = '\n'.join(parameters)
...
#				bm.write('arg = ' + argument + ';\n')
			...
			#		bm.write('\tfalsif_pb.max_time = '+ timeout + ';\n')
				...
				bm.write('\tlog = breachSampling(br, budget, phi, controlpoints, tspan, input_name, input_range, solver);\n')
